...
export class OPRETURNParser {
	...
	public async run(): Promise<void> {
		...
		const taskQueue = new Queue((_, cb) => {
			this.parsingTask()
				.then(() => cb())
				.catch(err => cb(err));
		});
	...
	private async parsingTask(): Promise<void> {
		try {
			...
			const limitHeight = parsingLimits.end.height;
			let currentBlockHash = parsingLimits.start.hash;
			let currentHeight = parsingLimits.start.height;

			while (this.enabled && currentHeight >= limitHeight) {
				const block = (await this.rpc.getBlock(currentBlockHash, 2)) as any;
				const opReturns = this.parseOPRETURNForBlock(block);

				this.logger.trace({ currentHeight, currentBlockHash });
				this.logger.trace(
				...
				try {
					...
					this.logger.debug({ hash: block.hash, height: block.height }, 'Done');
				} catch (error) {
					...
					this.logger.debug(
						{ hash: currentBlockHash, height: block.height },
						`Block with height ${block.height} has already been parsed. Skipping...`,
					);
				}
	...
	private async computeParsingLimits(): Promise<ParsingLimits> {
		...
		this.logger.debug(
			{ hash: bestBlockHash },
			`Best block hash is ${bestBlockHash}`,
		);
		this.logger.debug(
			{ height: bestBlockHeight },
			`Best block height is ${bestBlockHeight}`,
		);
		...
		this.logger.debug(
			...
			`Lowest indexed height is ${lowestIndexedHeight}`,
		);
		this.logger.debug(
			{ height: highestIndexedHeight },
			`Highest indexed height is ${highestIndexedHeight}`,
		);
